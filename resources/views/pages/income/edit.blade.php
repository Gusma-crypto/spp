...
@section('content')
    <div class="card-body">
        <form action="{{ route('master.income.update', $data->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="mb-3">
                <label for="total_income" class="form-label"><span class="text-danger">* </span>Jumlah Pemasukan</label>
                <input type="text" name="total_income" id="total_income" class="form-control" required value="{{ $data->total_income }}" />
            </div>
...
@section('script')
    <script>
        $(document).ready(function() {
            $('#total_income').on('keyup', function() {
                let value = $(this).val();
